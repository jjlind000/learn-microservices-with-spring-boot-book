...
/**
 * Retrieves a list of {@link LeaderBoardRow}s representing the
 Leader Board
 ...
 */
    @Query("""
            SELECT NEW microservices.book.java_22.gamification.game.domain.LeaderBoardRow(s.userId, SUM(s.score)) 
            FROM ScoreCard s 
            GROUP BY s.userId ORDER BY SUM(s.score) DESC
           """)
    List<LeaderBoardRow> findFirst10();
    /**
     * Retrieves all the ScoreCards for a given user, identified by his
     user id.
     *
     * @param userId the id of the user
